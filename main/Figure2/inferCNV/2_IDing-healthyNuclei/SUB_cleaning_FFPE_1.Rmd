...
For this section, all the nuclei look very similar, so we will input all of them as the reference
...
Selecting all the nuclei since they all look equally meh
```{r}
nodes.interest <- c(1)
...
st.cnt <- colnames(read_tsv(list.files("/Users/javierescudero/Documents/work/SIMPlex-seq/data/iCNV/Visium/counts_for_object/st_ref/mengiQC_whole",
                                      pattern =  "105_D1", full.names = TRUE)))
```

Reading the pathologist annotation for the Visium section
```{r}
st.anno <- read_csv(list.files(patho.dir, full.names = TRUE, pattern = "105_D1_Epithelial")) %>%
  drop_na(Epithelial)
...
# exporting
write_tsv(anno.df, 
          file = paste0(out_dir("anno"), "/V42L22-105_D1.tsv"), 
          col_names = FALSE)
```
