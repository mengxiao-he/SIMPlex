...
png(paste0(cnv.dir,"/iCNV_BC_1_1/phylo.png"),width=10000,height=2500, res = 300)
plot(dendrogram,show.tip.label = FALSE)
nodelabels(text=1:dendrogram$Nnode,node=1:dendrogram$Nnode+Ntip(dendrogram))
dev.off()
```

After looking at the dendrogram with the node numbers, I have decided that cells under node 2 look a bit sick. Therefore, I want to keep those that fall under node 123.
```{r}
nodes.interest <- c(123)
...
st.cnt <- colnames(read_tsv(list.files("/Users/javierescudero/Documents/work/SIMPlex-seq/data/iCNV/Visium/counts_for_object/st_ref/mengiQC_whole",
                                      pattern =  "302_A", full.names = TRUE)))
```

Reading the pathologist annotation for the Visium section
```{r}
st.anno <- read_csv(paste0(patho.dir, "/V52L18-302_A_Epithelial.csv")) %>% drop_na(Epithelial)
colnames(st.anno) <- c("Barcode", "anno")
st.anno <- filter(st.anno, Barcode %in% st.cnt) %>% 
  mutate(anno = paste(anno, "-V52L18-302_A", sep = ""))
```
...
# exporting
write_tsv(anno.df, 
          file = paste0(out_dir("anno"), "/V52L18-302_A.tsv"), 
          col_names = FALSE)
```
