...
# Summary
The goal of this markdown is to export an annotation file for only the **mammary epithelial** cluster of the FF nuclei data. These nuclei are the healthy cluster, so we should run inferCNV on them to truly select the healthy cohort that we can then use on the ST data
...
## Set up input data
So we want to run only for the nuclei annotated as diseased or as epithelial. In this seurat we have both samples, so we have to export annotation and counts for each sample
```{r}
