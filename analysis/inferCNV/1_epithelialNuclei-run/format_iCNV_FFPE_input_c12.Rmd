...
# Summary
The goal of this markdown is to rerun inferCNV for the latest annotation of the single-nuclei curated by Reza. This markdown is to prepare a run for the healthy cluster of the FFPE section of interest. So it will be a run of only the healthy nuclei and then we will see if we detect some CNVs to properly select the healthy nuclei
...
Reza manually annotated the clusters stored in *SCT_snn_res.0.8*. The proper name of the clusters is in *manual_annot_0.8*. I want only the epithelial nuclei
```{r}
# Section A
