...
Setting directories
```{r}
cnv.dir <- "/Users/javierescudero/Documents/work/SIMPlex-seq/results/inferCNV/iCNV_outs/single-nuclei/Rezas_manual-annot/ff/mammary"
patho.dir <- "/Users/javierescudero/Documents/work/SIMPlex-seq/data/visium/human_BC/FF/BC_FF_annotations"
out_dir <- function(i){
...
png(paste0(cnv.dir,"/iCNV_BC_1_2/phylo.png"),width=10000,height=2500, res = 300)
plot(dendrogram,show.tip.label = FALSE)
nodelabels(text=1:dendrogram$Nnode,node=1:dendrogram$Nnode+Ntip(dendrogram))
dev.off()
```

After looking at the dendrogram with the node numbers, I have decided that cells under node 2 look a bit sick. Therefore, I want to keep those that fall under node 52
```{r}
nodes.interest <- c(52)
...
st.cnt <- colnames(read_tsv(list.files("/Users/javierescudero/Documents/work/SIMPlex-seq/data/iCNV/Visium/counts_for_object/st_ref/mengiQC_whole",
                                      pattern =  "302_B", full.names = TRUE)))
```

Reading the pathologist annotation for the Visium section
```{r}
st.anno <- read_csv(paste0(patho.dir,"/Máté_V52L18-302_B.csv"))
colnames(st.anno) <- c("Barcode", "anno")
st.anno <- filter(st.anno, str_detect(anno, pattern = "Tumor-stroma|Tumor rich|DCIS|Normal ductal epithelium")) %>% 
  filter(Barcode %in% st.cnt)
```
...
# exporting
write_tsv(anno.df, 
          file = paste0(out_dir("anno"), "/V52L18-302_B.tsv"), 
          col_names = FALSE)
```
